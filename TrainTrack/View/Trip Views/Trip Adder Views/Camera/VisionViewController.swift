...
class VisionViewController: CameraCaptureViewController {
	...
	func recognizeTextHandler(request: VNRequest, error: Error?) {
		...
		for visionResult in results {
            ...
            if let result = candidate.string.extractCarriageCode() {
        ...
        show(boxGroups: [(color: UIColor.systemBlue.cgColor, boxes: greenBoxes)])
	...
	func show(boxGroups: [ColoredBoxGroup]) {
		DispatchQueue.main.async {
            guard let layer = self.previewView?.videoPreviewLayer else { return }
			self.removeBoxes()
